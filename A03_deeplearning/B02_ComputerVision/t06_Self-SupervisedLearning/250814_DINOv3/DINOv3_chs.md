...
<table>
<tr><td>Method</td><td>Arch.</td><td>FT</td><td>LoveDA</td><td>iSAID</td><td>DIOR</td></tr>
<tr><td rowspan="2">Prev.SotA</td><td rowspan="2"></td><td rowspan="2">🔥</td><td>BillionFM,ViT-G</td><td>SkySense V2,Swin-G*</td><td>SkySense V2,Swin-G*</td></tr>
<tr><td>54.4</td><td>71.9</td><td>79.5</td></tr>
<tr><td>Decoder Arch.</td><td></td><td></td><td>UPerNet</td><td>UPerNet</td><td>Faster-RCNN</td></tr>
<tr><td>Privthi-v2</td><td>ViT-H</td><td>🔥</td><td>52.2</td><td>62.8</td><td>-</td></tr>
<tr><td>DINOv3 Sat</td><td>ViT-L</td><td>❄️</td><td>54.4</td><td>62.9</td><td>72.7</td></tr>
<tr><td>DINOv3 Sat</td><td>ViT-7B</td><td>❄️</td><td>55.3</td><td>64.8</td><td>76.6</td></tr>
<tr><td>DINOv3 Web</td><td>ViT-7B</td><td>❄️</td><td>56.2</td><td>71.4</td><td>80.5</td></tr>
</table>
...
本节讨论了最近在大规模模型训练过程中观察到的伪影和异常值现象，这些现象在大语言模型（LLM）（An 等，2025）和视觉领域（Darcet 等，2024）中均有发现。借鉴 An 等（2025）的定义，异常值通常指网络激活值显著偏离其分布均值的情况。在 DINOv3 的训练过程中，我们发现了不同层级的此类异常值：一些出现在图像块（patch）层级，另一些则出现在特征维度层级。下文将详细讨论所观察到的不同类型异常值、它们对训练及结果的影响，以及我们尝试解决这些问题的方法和初步结论。

#### A.1 高范数图像patch异常值

![](images/246755f3deef227d83ab2f5f11317cf29067c7a834a4af822d86c73437d8dc87.jpg)
(a) 按不同异常值处理策略可视化patch范数。底部两行每行共享一个颜色映射，从深蓝色（低值）到黄色（高值）。
<table>
<tr><td>Outlier Strategy</td><td>IN1k (Linear)</td><td>ADE20k mIoU</td></tr>
<tr><td>∅</td><td>86.4</td><td>53.2</td></tr>
<tr><td>4 Registers</td><td>86.6</td><td>53.0</td></tr>
<tr><td>Attention Bias</td><td>86.5</td><td>52.7</td></tr>
<tr><td>Value Gating</td><td>86.3</td><td>52.2</td></tr>
</table>
(b) 定量消融实验。

图20：评估不同策略对缓解高范数patch异常值的影响，包括(a)定性结果和(b)定量结果。我们使用一个70亿参数的模型，按照我们的训练方案训练了15万次迭代，分别在不使用任何高范数处理策略（∅）、使用四个寄存器tokens（Darcet等，2024）、或采用注意力偏置和值门控策略（An等，2025）的情况下生成结果。在(a)的第一行中，我们绘制了三张图像的输出patch范数分布（按升序排列）。我们还可视化了不同异常值策略下每张图像的输出patch范数（底部两行），使用相同的颜色映射——每张图像的最小值和最大值是在不同异常值策略之间计算得出的。

Darcet 等（2024）发现，图像块异常值会负面影响 DINOv2 的性能。这些异常值主要表现为高范数（high-norm）的 token，通常出现在图像中信息量较低的背景区域。研究发现，这些 token 在图像块与 [CLS] token 之间的内部通信中扮演关键角色。此外，这种现象也影响其他模型，无论其是否经过监督训练，例如 CLIP（Radford 等，2021）。当我们将模型扩展至 70 亿参数（7B）时，也观察到了此类高范数图像块的出现，且主要集中在图像背景区域。本节展示了在 7B 模型上训练 15 万次迭代的结果，尽管训练轮数有限，但已为我们提供初步信号以指导后续决策。我们在图 20a 的 $\mathcal{∅}$ 列中绘制了输出图像块的范数（在层归一化之前），其中以黄色标示的高范数图像块出现在天空及其他低信息区域。

**Token Registers（Token 寄存器）**  
为了缓解此类 token 异常值的出现，Darcet 等（2024）提出了一种简单而有效的解决方案：在 ViT 的输入序列中引入额外的 token，称为“寄存器”（registers）。这些寄存器的作用是接管图像块与 [CLS] token 之间的内部通信。基于该研究的结论，我们在模型中使用了 4 个寄存器，并未进一步进行消融实验，原因在于实验成本较高。图 20a 展示了该策略的实际效果：高范数异常值被有效消除，这一点也通过范数分布的直方图得到了进一步验证。此外，我们在图 20b 中定量展示了在 ImageNet-1k（IN1k）基准测试上引入额外寄存器 token 所带来的性能提升。
...
在模型架构中引入额外的寄存器（registers）能够有效解决高范数图像块（patch）异常值的问题。然而，在训练7B规模模型的过程中，我们观察到一种新型的异常值现象：它并非出现在不同图像块之间，而是出现在所学习表征的特征（通道）维度内部。具体而言，通过对Transformer各层在训练过程中的图像块激活值进行分析，我们发现一小部分特征维度的幅值异常地大，尽管各图像块之间的范数保持稳定。有趣的是，这些特征维度异常值在不同的图像块和图像样本中持续表现出较高的数值，这种行为与（An等，2025）中报告的观察结果形成鲜明对比。此外，这些异常维度在给定模型的各个网络层中持续存在，其幅值随网络深度增加而逐步上升，并在输出层达到最大值。同时，这些异常值的幅值在整个训练过程中也持续增长。
...
本节提供了本文所用所有基准数据集和评估指标的详细描述。

#### D.1 语义分割：线性探针

**数据集与指标**  
我们通过在三个基准数据集上进行线性探针（linear probing）来评估 DINOv3 的语义分割性能：ADE20k (Zhou 等, 2017)、VOC12 (Everingham 等, 2012) 和 Cityscapes (Cordts 等, 2016)。报告的评估指标为标准的平均交并比（mean Intersection-over-Union, mIoU）。

**评估协议**  
为了评估密集特征的质量，我们在每个基准的训练集上训练一个线性分类器。该线性层施加于冻结主干网络的输出 patch 特征之上（在层归一化之后），并进一步使用一个可训练的批归一化（batch normalization）层对特征进行归一化。对于所有主干网络，我们使用 AdamW 优化器进行超参数搜索，学习率在 $\{ 1 \times 10^{-4}, 3 \times 10^{-4}, 1 \times 10^{-3} \}$ 中变化，权重衰减在 $\{ 1 \times 10^{-4}, 1 \times 10^{-3} \}$ 中变化。

#### D.2 深度估计：线性探针

**数据集与指标**  
我们在深度估计任务的两个基准数据集 NYUv2 (Silberman 等, 2012) 和 KITTI (Geiger 等, 2013) 上评估 DINOv3 特征在几何任务中的质量。结果使用均方根误差（Root Mean Squared Error, RMSE）作为评估指标。

**评估协议**  
为了评估密集特征的质量，我们在每个基准的训练集上训练一个线性分类器。该线性层施加于冻结主干网络的输出 patch 特征之上（在层归一化之后），并进一步使用一个可训练的批归一化层对特征进行归一化。对于所有主干网络，我们使用 AdamW 优化器进行超参数搜索，学习率在 $\{ 1 \times 10^{-4}, 3 \times 10^{-4}, 1 \times 10^{-3} \}$ 中变化，权重衰减在 $\{ 1 \times 10^{-4}, 1 \times 10^{-3} \}$ 中变化。

#### D.3 3D 关键点匹配

**数据集与指标**  
几何对应关系在 NAVI 数据集 (Jampani 等, 2023) 上进行评估，语义对应关系在 SPair 数据集 (Min 等, 2019) 上进行评估。对于 NAVI 数据集，我们将图像调整为边长为 448/512 像素（分别对应 patch 大小为 14/16 的模型）。对于 SPair 数据集，图像调整为边长为 896/1024 像素（分别对应 patch 大小为 14/16 的模型）。性能通过“对应召回率”（correspondence recall）来衡量，即在指定距离阈值内的对应点所占的百分比。

**评估协议**  
对于 NAVI 数据集，我们遵循 Probe3D (Banani 等, 2024) 中定义的协议。具体而言，我们对 1/4 的物体视角进行下采样，对每个源视角（source view），选择另一个在最大旋转角度 120 度范围内的目标视角（dest. view），构成图像对（source, dest.），用于 patch 匹配。对于每对图像，我们将源图像中的每个 patch（位于物体内部）与目标图像中的 patch 进行匹配。保留余弦相似度最高的前 1000 个匹配结果用于评估，并基于已知的相机位姿和两幅图像的深度图计算每个匹配的 3D 距离误差。由此可计算不同阈值下的召回率，我们使用的阈值为 1cm、2cm 和 5cm。最终，我们对多个阈值下的召回率取平均作为“对应召回率”。

对于每个被评估的主干网络，我们使用其最后一层的特征，并分别评估是否应用最终层归一化（final layer norm）的情况。这是因为我们发现某些模型在应用最终层归一化后性能显著下降。我们报告两种情况中的最佳结果。

#### D.4 无监督物体发现

**数据集与指标**  
本任务的目标是为每张图像生成一个边界框，以突出图像中出现的任意物体。我们遵循 Siméoni 等 (2021) 提出的无监督物体发现协议，在 VOC07 (Everingham 等, 2007)、VOC12 (Everingham 等, 2012) 和 COCO20K (Lin 等, 2014; Vo 等, 2020) 三个检测基准上评估所有主干网络。COCO20K 是 COCO2014 trainval 数据集 (Lin 等, 2014) 的子集，包含 19,817 张随机选取的图像，由 Vo 等 (2020) 首次提出，并被广泛用于此任务。每张图像生成一个边界框。评估使用“正确定位率”（Correct Localization, CorLoc）指标，即正确定位的边界框所占的百分比。若预测框与任意真实标注框的交并比（IoU）超过 0.5，则视为正确。

**评估协议**  
为了评估图像编码器的质量，我们采用 TokenCut 方法 (Wang 等, 2023c)。该方法将图像 patch 组织成一个全连接图，边表示 patch 对之间的相似性得分，该得分由 Transformer 主干网络学习到的特征计算得到。通过应用归一化割（Normalized Cut）算法求解图割问题，识别出显著物体的 patch，然后将边界框拟合到生成的显著物体掩码上。所有图像以原始分辨率输入编码器，我们使用主干网络输出的 patch 特征。为适应不同模型间特征分布的差异，我们对 TokenCut 唯一的超参数——图构建中的相似性阈值——进行搜索。具体地，我们将该阈值在 0 到 0.4 之间以 0.05 为步长进行遍历。

#### D.5 视频分割与跟踪

**数据集与指标**  
本任务使用 DAVIS 2017 (Pont-Tuset 等, 2017)、YouTube-VOS (Xu 等, 2018) 和 MOSE (Ding 等, 2023) 数据集。DAVIS 数据集包含 60 个训练视频和 30 个验证视频，所有帧均标注了实例分割的真实掩码。YouTube-VOS 的训练集有标注且公开，而验证集需通过评估服务器访问。为模拟 DAVIS 的设置，我们从标注视频中随机选取 2758 个（80%）作为训练集，其余 690 个（20%）作为验证集。类似地，我们将 MOSE 数据集划分为 1206 个验证视频和 301 个测试视频。对于所有数据集，我们使用标准的 $\mathcal{J}\&\mathcal{F}$-mean 指标 (Perazzi 等, 2016) 进行评估，该指标结合了区域相似性（$\mathcal{J}$）和轮廓准确性（$\mathcal{F}$）得分。仅对第一帧中标注的物体进行跟踪和评估，后续出现的物体即使有真实标注也忽略。
...
**数据集与评估指标**  
我们采用广泛使用的线性探针（linear probing）评估方法来评估DINOv3模型的整体质量。我们在ImageNet-1k训练集（Deng等，2009）上训练一个线性分类器，并在验证集上评估结果。为了评估模型的泛化能力，我们将模型迁移到多个分类测试集上进行评估：ImageNet-V2（Recht等，2019）和ReaL（Beyer等，2020），它们提供了ImageNet的替代图像和标签，旨在检验模型是否在原始ImageNet验证集上过拟合。此外，我们还考虑Rendition（Hendrycks等，2021a）和Sketch（Wang等，2019）数据集，它们包含ImageNet类别的风格化和人工绘制版本；Adversarial（Hendrycks等，2021b）和ObjectNet（Barbu等，2019）数据集，包含刻意设计的具有挑战性的样本；以及Corruptions（Hendrycks和Dietterich，2019）数据集，用于衡量模型对常见图像损坏的鲁棒性。对于所有数据集，我们报告top-1分类准确率作为评估指标，但ImageNet-C除外，对于该数据集我们报告平均损坏误差（mCE，参见Hendrycks和Dietterich，2019）。

对于细粒度数据集，我们采用与Oquab等（2024）相同的12个数据集，我们将其称为Fine-S：Food-101（Bossard等，2014）、CIFAR-10（Krizhevsky等，2009）、CIFAR-100（Krizhevsky等，2009）、SUN397（Xiao等，2010）、StanfordCars（Krause等，2013）、FGVC Aircraft（Maji等，2013）、VOC 2007（Everingham等，2007）、DTD（Cimpoi等，2014）、Oxford Pets（Parkhi等，2012）、Caltech101（Fei-Fei等，2004）、Flowers（Nilsback和Zisserman，2008）和CUB200（Welinder等，2010），以及更大的数据集Places205（Zhou等，2014）、iNaturalist 2018（Van Horn等，2018）和iNaturalist 2021（Van Horn等，2021）。

**评估协议**  
对于较大的数据集ImageNet、Places205、iNaturalist 2018和iNaturalist 2021，我们采用以下流程。对于每个基线模型，我们在CLS token的最终特征（经过层归一化后）上训练一个线性分类层，使用ImageNet-1k训练集。具体而言，我们使用带动量的SGD（动量为0.9），训练10个epoch，批大小为1024。我们对学习率进行网格搜索，候选值为：{1×10⁻⁴, 2×10⁻⁴, 5×10⁻⁴, 1×10⁻³, 2×10⁻³, 5×10⁻³, 1×10⁻², 2×10⁻², 5×10⁻², 1×10⁻¹, 2×10⁻¹, 5×10⁻¹, 1×10⁰, 2×10⁰, 5×10⁰}，权重衰减值为{0, 1e⁻⁵}，并使用ImageNet-1k的验证集选择最佳超参数组合。训练过程中，我们采用随机缩放裁剪（random resize crop）增强，使用标准的Inception裁剪参数。

对于Fine-S中的数据集，我们遵循Oquab等（2024）的做法，采用更轻量级的评估方式：使用scikit-learn库中的LogisticRegression实现，并采用L-BFGS求解器。

在两种情况下，我们都将模型在生成1024个patch token的分辨率下进行评估，即对于patch size为14的模型使用448×448分辨率，对于patch size为16的模型使用512×512分辨率。图像首先被缩放，使短边等于目标边长，然后取中心正方形裁剪区域。

#### D.8 实例识别

**数据集与评估指标**  
我们使用 Oxford 和 Paris 数据集进行地标识别（Radenović 等，2018），使用 Met 数据集（包含大都会艺术博物馆的艺术作品，Ypsilantis 等，2021），以及 AmsterTime 数据集（由现代街景图像与阿姆斯特丹历史档案图像配对组成，Yildiz 等，2022）。在表 9 中，我们报告了 Oxford-Hard、Paris-Hard 和 AmsterTime 的平均精度均值（mAP），以及 Met 的全局平均精度（GAP）。在表 23 中，我们额外提供了 Oxford-Medium 和 Paris-Medium 的 mAP，以及额外的评估指标 GAP- 和准确率（参见 Ypsilantis 等，2021）。对于 Oxford 和 Paris 数据集，我们将所有图像调整大小，使较长边为 224 像素，保持宽高比，然后进行全图中心裁剪，得到分辨率为 $224 \times 224$ 的图像。对于 AmsterTime，我们将所有图像调整大小，使较短边为 256 像素，保持宽高比，然后进行 $224 \times 224$ 的中心裁剪。对于 Met 数据集，我们在接近原始分辨率的情况下评估所有图像，将两个尺寸调整为最接近 patch 尺寸的倍数（当 patch 尺寸为 14/16 时，长边分别为 508/512）。
...
**数据集与评估指标**  
我们在 COCO（Lin 等，2014）和 COCO-O（Mao 等，2023）数据集上评估 DINOv3 的目标检测性能。COCO 是一个标准的目标检测基准，涵盖 80 个物体类别，包含 11.8 万张训练图像和 5 千张验证图像。COCO-O 是一个仅用于评估的数据集，类别与 COCO 相同，但视觉条件更具挑战性，例如存在严重遮挡、背景杂乱和光照变化等场景。为了训练目标检测模型，我们还利用了 Objects365（Shao 等，2019）数据集，该数据集包含 250 万张图像，涵盖 365 个物体类别，其中一部分直接映射到 COCO 的类别。对于 COCO 和 COCO-O，我们报告在 IoU 阈值 [0.5:0.05:0.95] 下计算的平均精度均值（mAP）。

**模型架构**  
我们的方法基于 Plain-DETR（Lin 等，2023b）实现，并进行了若干修改。我们不将 Transformer 编码器融合进主干网络，而是将其作为独立模块保留，类似于原始的 DETR（Carion 等，2020）。这使得我们可以在训练和推理过程中完全冻结 DINOv3 主干网络，使其成为首个实现这一点且具备竞争力的检测模型。我们从 DINOv3 ViT-7B/16 主干网络中提取四个中间层的特征，即 [10, 20, 30, 40] 层。对于每个 patch，我们将中间特征按通道拼接，得到 $4 \cdot 4096 = 16384$ 维的特征，再通过下文所述的窗口策略进一步扩展维度。主干网络提取的特征输入到编码器，编码器由 6 个自注意力模块堆叠而成，嵌入维度为 768。解码器由 6 个交叉注意力模块堆叠而成，具有相同的嵌入维度，其中 1500 个“一对一”查询和 1500 个“一对多”查询关注编码器的 patch token，用于预测边界框和类别标签。

**图像预处理**  
训练分为三个阶段，如下面所述，其中一阶段使用 1536 像素的基础图像分辨率，另两个阶段使用 2048 像素。遵循 DETR 的做法，我们应用随机水平翻转（$p = 0.5$），然后进行以下操作之一：(i) 随机缩放，将最短边在 920 像素（或 1228 像素）与当前阶段的基础分辨率（1536 或 2048）之间均匀采样；或 (ii) 随机裁剪，保留原始图像面积的 60%-100%，然后按 (i) 方式进行缩放。在评估时，图像被缩放至最短边为 2048 像素，不进行额外增强，且两边均向上取整到最接近 patch 尺寸的倍数。

**窗口策略**  
我们采用一种窗口策略，结合图像的全局视图和局部小视图，使主干网络能够处理各种尺度的物体。窗口数量固定为 $3 \times 3$，其大小根据输入分辨率变化。例如，对于尺寸为 $1536 \times 2304$ 的图像：

1. 图像被划分为 $3 \times 3$ 个不重叠的窗口，每个窗口大小为 $512 \times 768$。每个窗口通过主干网络前向传播，生成 $32 \times 48$ 个维度为 16384 的 patch token。所有窗口的特征在空间上重新组装成 $(3 \cdot 32) \times (3 \cdot 48)$ 的特征图。
2. 整张图像被缩放到 $512 \times 768$ 并通过主干网络，生成 $32 \times 48$ 个维度为 16384 的 patch token 特征图。然后通过双线性插值上采样至 $96 \times 144$，与步骤 1 的窗口化特征图尺寸匹配。
3. 最后，将步骤 1 和步骤 2 的特征图按通道拼接，得到 $96 \times 144$ 的特征图，维度为 $2 \cdot 16384 = 32768$。该特征图被展平为 $96 \times 144$ 个 token 的序列，并输入编码器。

**训练过程**  
我们采用三阶段训练课程，在 Objects365（Shao 等，2019）和 COCO（Lin 等，2014）数据集上逐步提升分辨率。整个训练过程中使用 AdamW 优化器（Loshchilov 和 Hutter，2017），权重衰减为 0.05。遵循 DETR 的做法，分类损失使用 Focal Loss（Lin 等，2018），权重为 2；边界框损失使用 L1 损失，权重为 1；并辅以 GIoU（Rezatofighi 等，2019）损失，权重为 2。各阶段如下：

1. 首先在 Objects365 上以 1536 像素为基础分辨率开始训练。训练 22 个 epoch，全局批量大小为 32，分布在 32 个 GPU 上。初始预热 1000 步后，学习率设为 $5 \cdot 10^{-5}$，并在第 20 个 epoch 后除以 10。
2. 接着在 Objects365 上以 2048 像素为基础分辨率继续训练。训练 4 个 epoch，学习率为 $2.5 \cdot 10^{-5}$。
3. 最后在 COCO 上以 2048 像素为基础分辨率完成训练。进行 12 个 epoch 的训练，前 2000 次迭代采用线性预热，学习率随后按余弦衰减调度，从 $2.5 \cdot 10^{-5}$ 下降到第 8 个 epoch 的 $2.5 \cdot 10^{-6}$。在此阶段，我们使用 IA-BCE 分类损失（Cai 等，2024）替代 DETR 的简单 Focal Loss。我们发现该损失在迁移学习阶段能提升模型性能，但在预训练阶段效果不明显。由于该损失融合了类别和边界框信息，只有在边界框预测已良好初始化时才能发挥最大作用。此阶段 GIoU 损失权重设为 4，以鼓励更好的边界框对齐。

**测试时增强（TTA）**  
在测试时，我们遵循上述推理流程，将图像缩放至短边为 1536 或 2048 像素。在此两种分辨率下，COCO 的 mAP 分别为 65.4 和 65.6。或者，我们可以采用 Bolya 等（2025）提出的测试时增强（TTA）策略，即对图像进行翻转并缩放到多个分辨率，然后使用 SoftNMS（Bodla 等，2017）合并预测结果。具体而言，每张图像在 [1536, 1728, 1920, 2112, 2304, 2496, 2688, 2880] 多个分辨率下处理，最终 mAP 达到 66.1。

#### D.10 语义分割

**数据集与评估指标**  
我们在 ADE20k (Zhou 等, 2017)、Cityscapes (Cordts 等, 2016)、COCO-Stuff (Caesar 等, 2018) 和 VOC 2012 (Everingham 等, 2012) 数据集上对 DINOv3 进行语义分割评估。ADE20k 是一个广泛使用的语义分割基准数据集，包含 150 个语义类别，涵盖从户外场景到室内人物和物体的图像。此外，我们使用 COCO-Stuff 和 Hypersim (Roberts 等, 2021) 数据集对模型进行预训练。COCO-Stuff 是一个比 ADE20k 更大的数据集（包含 11.8 万张训练图像），包含 80 个“物体”（thing）类别和 91 个“景物”（stuff）类别；而 Hypersim 是一个逼真的合成室内场景数据集，包含 40 个语义类别，其标注更加清晰准确。超过一半的 Hypersim 图像包含 21 个或更多对象，使其成为帮助模型学习丰富场景信息的理想选择。所有数据集的评估指标均为平均交并比（mIoU）。

**模型架构**  
我们采用了其他基线方法所使用的 ViT-Adapter 和 Mask2Former 配置，但做了若干调整。首先，为了确保我们的主干网络保持冻结状态且其激活值不被修改，我们移除了 ViT-Adapter 中的 injector（注入器）组件。这使得主干网络输出的特征可直接用于 extractor（提取器）模块。其次，由于我们的主干网络输出维度为 4096，而其他基线模型的主干网络输出维度通常为 1024 或 1536，因此我们将 Mask2Former 解码器中的嵌入维度从默认的 1024 扩展至 2048。作为解码器的输入，我们从 DINOv3 7B/16 主干网络的四个中间层（即第 [10, 20, 30, 40] 层）提取特征。我们对所有层的特征应用最终的层归一化（layer norm），并添加可学习的批归一化（batch normalization）。

**训练协议**  
在 COCO-Stuff 上生成结果时，我们采用余弦学习率调度器（cosine scheduler），包含 6k 步的线性预热（warmup），最大学习率为 1.5e-5，训练总步数为 80k，输入图像分辨率为 1280 像素。对于其他数据集（ADE20k、Cityscapes 和 VOC 2012）的训练，我们首先在 COCO-Stuff 上对解码器进行 80k 步的预训练，同样使用 6k 步线性预热和 1.5e-5 的学习率，遵循余弦调度策略。由于 COCO-Stuff 包含更多样化的语义类别（共 171 类）且数据量更大，有助于模型学习更丰富的语义信息。随后，我们在 Hypersim 数据集上继续训练 10k 步，学习率为 2.5e-5，采用余弦调度器并配有 1.5k 步的线性预热。这一阶段大约相当于 2 个训练周期，得益于 Hypersim 数据集高度逼真的合成特性，有助于模型学习高质量的图像到掩码对应关系。最后，我们在 ADE20k 上训练 20k 步，学习率为 3e-5，同样使用 1.5k 步线性预热和余弦学习率调度。我们在验证集上报告最终结果。对于 Cityscapes 和 VOC 2012，分别使用 1.5e-5 和 1e-5 的学习率。所有训练均使用 AdamW 优化器，批量大小（batch size）为 16。

**推理过程**  
对于单尺度评估，我们采用滑动窗口推理方法（sliding inference）。具体而言，首先将图像调整至训练时的分辨率（例如，在 ADE20k 上训练时分辨率为 896 像素，因此一张 $400 \times 500$ 的图像会被调整为 $896 \times 1120$ 像素）。然后使用滑动窗口方法，在固定大小的方形裁剪区域（例如 $896 \times 896$ 像素）上进行预测，滑动步长为 596 像素（以 ADE20k 为例），逐块生成预测结果。之后将这些局部预测结果聚合，并重新缩放回原始图像尺寸，得到最终的分割结果。

对于测试时增强（test-time augmentation, TTA），ADE20k 和 VOC 2012 的图像被分别缩放为评估分辨率的 [0.9, 0.95, 1.0, 1.05, 1.1] 比例，并对每张图像进行水平翻转，从而每个样本生成共 10 个预测结果。在每个图像上完成滑动窗口推理后，将结果重新缩放到原始图像尺寸并取平均。COCO-Stuff 164K 的 TTA mIoU 通过为每个样本额外添加一次水平翻转图像获得；而对于 Cityscapes，则使用评估分辨率的 [1.0, 1.5, 2.0] 三种缩放比例进行测试增强。
